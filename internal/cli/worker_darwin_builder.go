...
// buildDarwinHostWorkerGuardScript builds the on-disk script that
// launchd's /bin/sh invokes. It changes into the site/worktree dir and
// exec's the worker command through fnm so the requested node version
// is on PATH (vite + npm run dev are resolved against that node).
//
// PATH is seeded with Homebrew + system bins so fnm's internal lookups
// (it uses tar / unzip / etc. for some commands, and shells out to the
// installed node) succeed under launchd's minimal default env. Single
// quotes inside command / sitePath are escaped via the standard
// '"'"' idiom so arbitrary user-provided commands survive the wrapper.
func buildDarwinHostWorkerGuardScript(fnmBin, nodeVersion, sitePath, command string) string {
...
cd '%s' || exit 1
exec '%s' exec --using=%s -- /bin/sh -c '%s'
`, escapedPath, fnmBin, nodeVersion, escapedCmd)
}
