...
- **Desktop notifications card**: master switch + per-category toggles (mail captured, worker failures, finished service operations, service updates, dumps), a *Send a test notification* button, and the list of subscribed browsers with *Forget* actions. See [Desktop notifications](./notifications.md).
