...
- **macOS parity for v1.19 features that were Linux-only**. Worker self-heal, the dashboard's failed-worker banner, and the `workers_health` MCP tool now see real failed-unit state on macOS. `siteinfo.AllUnitStates` was returning an empty map on darwin, so `workerheal.Detect` never fired. The `UnitLifecycle` interface gained `AllUnitStates()`, the `darwinServiceManager` walks `~/Library/LaunchAgents/lerd-*.plist` to populate it, and `siteinfo/unitcache_darwin.go` plugs the launchd walker in via a new `allUnitStatesFn` hook. The "last error" excerpt for failed workers also works on macOS now via a platform-split `readLastError` that tails `~/Library/Logs/lerd/<unit>.log` (where the launchd plist redirects stdout and stderr) instead of relying on `journalctl`.
...
- **Tinker tab works on Laravel projects with non-trivial vendor trees**. PHP CLI's 128 M `memory_limit` default was exhausted by `ClassAliasAutoloader` during boot (it requires the full composer class map upfront, FPM-served HTTP requests don't hit this because they only autoload per request). Tinker's podman exec now passes `-d memory_limit=512M`. Also sets `PSYSH_TRUST_PROJECT=1` to silence PsySH's non-interactive "Restricted Mode" warning that otherwise broke simple expressions like `echo env('APP_URL');` because the autoload skip left `env()` bound to its bare-PHP fallback. The trust prompt only protects against running PsySH inside a malicious cloned repo, which doesn't apply when the user is explicitly running their own project's Tinker tab from the lerd UI.
- **PostGIS preset on Apple Silicon now uses the upstream image instead of the imresamu fork**. The official `postgis/postgis` manifest publishes only amd64 entries; earlier releases swapped in the unofficial `imresamu/postgis` multi-arch fork via a YAML `platform_overrides` hook, but legacy or hand-edited configs pinning the non-alpine tag (`postgis/postgis:16-3.5`) slipped through the alpine-only `image_match` and crashed. The fork is gone; on darwin a new `podman.PlatformPodmanArgs` hook injects `PodmanArgs=--platform=linux/amd64` into the rendered quadlet so podman pulls the amd64 manifest and Podman Machine runs the container under qemu user-mode (or Rosetta when the VM has it wired). Hooked centrally at `WriteQuadletDiff` so cli, UI, MCP, and install all emit byte-identical units. Existing data dirs initialised under the imresamu fork open cleanly on the upstream image (both ship PG 16 + PostGIS 3.5); users may want to run `ALTER EXTENSION postgis UPDATE` once after the swap to align in-database stored procs with the slightly newer C library. Pinning a multi-arch fork explicitly in `~/.config/lerd/config.yaml` (e.g. `image: docker.io/imresamu/postgis:16-3.5-alpine`) bypasses the platform pin entirely, only `postgis/postgis` triggers the swap.
...
- **`lerd worktree add` and `lerd worktree remove` interactive wrappers** mirroring `git worktree`'s subcommand layout, with every flag passing straight through to git. `add` polls until the watcher's install pipeline (composer + npm) finishes, then prompts via `huh.NewSelect` for the production-build script to run (skip / `build` / `prod` / `build:prod` / `build-prod` / `production`, whichever exist in `package.json`), and for the database setup (share parent / isolated empty / clone from main / clone from another isolated worktree). When the worktree's branch already has a preserved isolated DB the prompt prepends two extra options at the top — **Reuse preserved isolated DB** (reconnects without touching the schema) and **Reset preserved DB to a fresh empty schema** — so re-adding the same branch picks up where you left off or wipes the slate, your call. Picking "isolated empty" then offers to run `php artisan migrate --force` against the new schema. `remove` runs `git worktree remove`, recovers from "modified or untracked files" by offering a force-retry select prompt, then asks at the end whether to drop the worktree's isolated database (default **Keep**, so re-adds reconnect to the same data). LAN-share is intentionally not prompted in either command; it's a niche flag the user can flip in the dashboard. See [Git worktrees → `lerd worktree add` and `lerd worktree remove`](features/git-worktrees.md#lerd-worktree-add-and-lerd-worktree-remove).
- **Opt-in per-worktree database isolation in the dashboard**. The site controls now expose an **Isolated DB** toggle whenever a worktree is active and the parent uses a lerd-managed mysql, mariadb, or postgres service. Flipping it on creates `<parent_db>_<sanitized_branch>` in the same service container, rewrites `DB_DATABASE` in the worktree's `.env`, and persists the choice as `db_isolated: true` in the worktree's `.lerd.yaml` (so it travels with the branch in git). On enable, lerd asks how the new schema should be seeded: empty, cloned from the parent (`mysqldump --single-transaction | mysql` or `pg_dump | psql` end-to-end inside the service container), or cloned from another already-isolated worktree. Cleanup ordering is vhost first, LAN-share second, isolated database last — `lerd worktree remove` prompts about the database explicitly (Keep is the default), and plain `git worktree remove` leaves it for either the prompt path or the daemon's `scanWorktrees` startup sweep. Off by default — existing worktrees keep sharing the parent's DB until the user opts in. See [Git worktrees → Per-worktree database](features/git-worktrees.md#per-worktree-database).
- **Per-worktree LAN-share proxy** with a per-user registry at `~/.local/share/lerd/worktree-lan-shares.yaml`. The dashboard's LAN toggle is worktree-aware; `LANShareStartWorktree` / `LANShareStopWorktree` allocate ports across the same pool the parent uses, the proxy targets `<branch>.<parent>.test` so nginx routes correctly, and the QR-code popover passes `?branch=` so the encoded URL is the worktree's. Cleanup on `git worktree remove` POSTs `lan:unshare?branch=` to lerd-ui so the listener closes in the daemon's process (not the watcher's, which holds an empty server map).
- **Watcher race fix for fresh worktrees**: `handleNewEntry` now polls `HEAD` until it's a final ref or SHA before firing `onAdded`, closing a window where fsnotify Created the entry dir before git finalised HEAD. Without this, `git worktree add main` would write `detached.<site>.test.conf` for the transient detached HEAD, never get a Change event when HEAD was atomically updated to `main`, and leave the URL routing to whatever vhost won the Host-header fallback (typically the parent's). The daemon's startup `scanWorktrees` pass also reconciles stale `*.<site>.test.conf` files now, so a worktree removed while the watcher was offline gets its orphan vhost dropped on next start.
...
- **Worktree-scoped site detail view in the dashboard, with per-worktree PHP and Node versions**. The site detail panel no longer stacks a row per worktree under the header. The path line now carries an inline branch picker (`/path · git:(main) 3 ▾`) that collapses to a single line regardless of worktree count and opens a dropdown listing the main checkout and every active worktree with its derived domain. Picking a branch re-scopes the rest of the panel to that worktree: the site title and the **Open** / **Terminal** buttons target the worktree's domain and checkout path, the **App logs** tab tails `storage/logs` from the worktree's directory (`/api/app-logs/{domain}?branch=<sanitized>` on the backend), and the PHP / Node version selectors show the worktree's effective version with a dashed violet border when the value is inherited from the parent. Changing a selector while a worktree is active writes a worktree-only override to `.lerd.yaml` inside the worktree's checkout, so the choice travels with the branch in git. The override is honoured everywhere lerd materialises worktree state on disk: vhost generation on add, rename, pause/unpause, and `lerd secure` / `lerd unsecure`. Worktrees with no `.lerd.yaml` override continue to inherit the parent's PHPVersion and NodeVersion. See [Git worktrees → Web UI](features/git-worktrees.md#web-ui).

---

## [1.19.0-beta.3] — 2026-05-01

### Added

- **Tinker tab, in-browser PHP REPL per site with autocomplete and live linting** (#282). Adds a Tinker tab to the site detail page backed by CodeMirror, context-aware autocomplete (project models from PSR-4, composer helpers, PHP built-ins cached per version, framework hints, buffer variable completion), live `php -l` linting (debounced 2.5 s, symbol cache per domain to keep lerd-ui CPU flat under heavy typing), and an editor-styled output panel with collapsible Symfony VarDumper trees. Driven by the framework definition's `tinker:` block (`command`, `execute_flag`, `execute_positional` for drush/wp-cli style invocations, `requires_package`, `requires_file`); Laravel ships with one bundled, sites without a framework REPL run a temp script under plain `php` with composer autoload included. Bare expressions auto-dump per statement, the output is split on a record separator so each top-level statement renders as its own block. See [Tinker tab](features/tinker.md).
